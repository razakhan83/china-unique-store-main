...
export default function ProductWishlistButton({ product, mode = 'grid', className = '' }) {
  ...
  return (
    <button
      ...
      disabled={isSubmitting || isLoading}
      className={cn(
        "group/wishlist absolute right-2.5 top-2.5 z-10 hidden size-9 items-center justify-center rounded-full border border-border/60 bg-background/92 p-0 text-foreground shadow-xs backdrop-blur-md outline-none transition-all duration-300 ease-out hover:scale-[1.1] hover:-translate-y-0.5 active:scale-[0.96] focus-visible:ring-2 focus-visible:ring-ring/50 md:inline-flex md:hover:border-red-500 md:hover:bg-red-50 md:hover:text-red-500 md:hover:shadow-[0_4px_12px_rgba(239,68,68,0.2)] after:absolute after:-inset-2 after:content-['']",
        active
    ...
    >
      <span className="relative block size-4.5">
        <Heart
          className={cn(
            'absolute inset-0 size-4.5 transition-all duration-300 ease-out md:group-hover/wishlist:text-red-500',
            active ? 'scale-75 opacity-0' : 'scale-100 opacity-100'
          )}
        />
        <Heart
          className={cn(
            'absolute inset-0 size-4.5 fill-red-500 stroke-red-500 transition-all duration-300 ease-out',
